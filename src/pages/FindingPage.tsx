...
function MethodRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-4 px-5 py-4">
      <dt className="w-[72px] shrink-0 text-[13.5px] font-medium leading-[1.7] text-ink-3">{label}</dt>
      <dd className="min-w-0 flex-1 text-[15px] leading-[1.7] text-ink-2">{value}</dd>
    </div>
  )
}
  ...
  if (!f) {
    return (
      <div className="card p-6 text-center">
        <p className="text-[15px] font-medium text-ink">未找到这条研究</p>
  ...
  return (
    <article className="animate-rise space-y-6">
      <header className="card p-5">
        ...
        {f.en && <p className="mt-1.5 text-[13.5px] leading-[1.7] text-ink-3">{f.en}</p>}
        <p className="mt-2.5 text-[13.5px] leading-[1.7] text-ink-2">
          {f.design}
          {f.n ? ` · ${f.n}` : ''}
        </p>
      ...
      <section className="card overflow-hidden">
        <h3 className="flex items-center gap-2.5 border-b border-line bg-surface-2 px-5 py-3.5 text-[16.5px] font-semibold text-ink">
        ...
        <a
          href={href('/methods')}
          className="flex cursor-pointer items-center gap-1.5 border-t border-line bg-surface-2 px-5 py-3.5 text-[13.5px] text-accent transition-opacity duration-200 hover:opacity-80"
        >
          这些方法学信息该怎么读
          <ArrowRight size={14} />
        </a>
      ...
      <section className="card overflow-hidden">
        ...
        <ul className="row-divide">
          {f.results.map((r, i) => (
            <li key={r} className="flex gap-3 px-5 py-4">
              <span className="mt-[3px] flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent-soft text-[12.5px] font-semibold tabular-nums text-accent">
                {i + 1}
              </span>
              <p className="text-[15px] leading-[1.7] text-ink-2">{r}</p>
            </li>
          ))}
        </ul>
      </section>
      ...
      <section className="rounded-[14px] border border-line bg-surface-2 p-4">
        <h3 className="text-[13.5px] font-semibold uppercase tracking-wide text-ink-3">临床意义</h3>
        <p className="mt-1.5 text-[15px] leading-[1.7] text-ink">{f.impact}</p>
        {f.vsGuide && (
          <p className="mt-2.5 flex gap-2 rounded-xl bg-surface px-3 py-2 text-[13.5px] leading-[1.7] text-ink-2">
      ...
      <section className="rounded-[14px] border border-line bg-surface p-4">
        ...
        <ul className="mt-1.5 space-y-1 text-[13.5px] leading-[1.7] text-ink-2">
          <li>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5">
          {f.url && (
            <a
              ...
              className="inline-flex cursor-pointer items-center gap-1 text-[13.5px] text-accent transition-colors duration-200 hover:underline"
            >
              检索原文
